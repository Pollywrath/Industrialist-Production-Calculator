...
export const coal_power_plant_01: SpecialRecipe = {
  ...
  compute: (settings) => {
    const controls = {
      coalPct: clamp(readNumber(settings, 'coal_feed', 60), 0, 100),
      airPct: clamp(readNumber(settings, 'air', 92.5), 0, 100),
      exhaustPct: clamp(readNumber(settings, 'exhaust', 20), 0, 100),
    };
    const siloWater = clamp(readNumber(settings, 'silo_water', 200000), MIN_SILO_WATER, MAX_SILO_WATER);
    const incomingWaterTemp = readNumber(settings, 'distilled_water_temp', AMBIENT_TEMP);
