...
import { 
  DRILL_HEADS, CONSUMABLES, getAvailableDepths, 
  buildDrillInputs, buildDrillOutputs, calculateDrillMetrics 
    ...
    <div className="drill-settings-overlay" onClick={onClose}>
      <div className="drill-settings-bubble" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="drill-settings-content">
          ...
          {/* Display calculated metrics when all required settings are provided */}
          {metrics && (
            <div 
                  ...
                  <div>
                    ...
                    <div style={{ color: '#999', lineHeight: '1.5', fontSize: '12px' }}>
                      {outputs.map((output, idx) => (
                        <div key={idx}>
                          {(typeof output.quantity === 'number' ? output.quantity.toFixed(4) : output.quantity)}x {getProductName(output.product_id, getProduct)}
                        </div>
                      ))}
                    </div>
                  </div>
